function renderVertex(
  ...
  if (!row) row = createSemestersRow(rowId);
  if (!cell) cell = createCell(row, colId, vertex, optionalId);
...
function renderEmptyCell(rowPtr, currentCellPtr) {
  ...
  if (!cell) cell = createCell(row, colId, null);
...
function highlightCell(cell, vertex, optionalId) {
  ...
  while (siblingVertex) {
    ...
    if (dirWeight === DEPENDENT) elem.dataset.required = true;
    else if (dirWeight === REQUIRED) elem.dataset.dependent = true;
