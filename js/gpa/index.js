...
function toggleHintMessageMode(mode = "first") {
  ...
  const firstModeMessage =
    "Click any <span id='subject-explain'>subject</span> to add it, or the <span id='subheader-explain'>blue bordered rectangle</span> to select a <b>column</b>";
  const secondModeMessage =
    "Choose the any <span id='old-subject-explain'>subject</span> to add it, or <span id='old-subheader-explain'>red bordered rectangle</span> to select a <b>column</b>";

  hint_message.innerHTML =
    mode === "first" ? firstModeMessage : secondModeMessage;

  const subheader_explain = document.getElementById(
    mode === "first" ? "subheader-explain" : "old-subheader-explain",
  );

  subheader_explain.addEventListener("mouseenter", (e) => {
    const subheaders = document.querySelectorAll(
      `.${modesMap[mode]["subheader_to_be_selected_class"]}:not(.${modesMap[mode]["subheader_selected_class"]})`,
    );
...
function enableSubjectsSelectOnTable() {
  const semestersSubHeaders = [
    ...document.getElementsByClassName("flowsheet-subheader-cell"),
  ];
  const cells = [...document.getElementsByClassName("cell")].filter(
    (cell) => !cell.dataset.isEmpty,
  );
  ...
  cells.forEach((cell) => {
    ...
    } else cell.classList.add("cell_gpa");
  });
}
...
function enableOldSubjectsSelectOnTable() {
  const semestersSubHeaders = [
    ...document.querySelectorAll(
      ".flowsheet-subheader-cell:not(.cell-subheader-gpa_selected)",
    ),
  ];
...
function disableSelectOnTable() {
  const hint_message = document.getElementById("select-hint-message");
  const semestersHeaders = [
    ...document.getElementsByClassName("flowsheet-header-cell"),
  ];
  const semestersSubHeaders = [
    ...document.getElementsByClassName("flowsheet-subheader-cell"),
  ];
