...
function toggleSemestersHeadings(actions) {
  ...
  const {
    dataset: { semester: subjectSemester },
  } = actions.element;
  const subjectSemesterNumber = Number(subjectSemester);
  const selectedModeClass = `${selectType}_selected_class`;
  const selectedCurrentSemester = document.querySelectorAll(
    `.${modesMap[mode][selectedModeClass]}[data-semester="${subjectSemesterNumber}"]:not([data-optional-id],[data-is-training])`,
  );
  const otherMode =
    mode === "first" ? "second" : mode === "second" ? "first" : "";
  const allCurrentSemester = document.querySelectorAll(
    `[data-semester="${subjectSemesterNumber}"]:not([data-optional-id],[data-is-training])`,
  );
  const currentSubjectSemesterHeader = document.querySelector(
    `[data-header-semester="${subjectSemester}"]`,
  );

  const otherSemesterNumber =
    subjectSemesterNumber % 2 === 0
      ? subjectSemesterNumber - 1
      : subjectSemesterNumber + 1;
  const yearNumber = Math.trunc((subjectSemesterNumber + 1) / 2);
  const selectedOtherSemester = document.querySelectorAll(
    `.${modesMap[mode][selectedModeClass]}[data-semester="${subjectSemesterNumber}"]:not([data-optional-id],[data-is-training])`,
  );
  const allOtherSemester = document.querySelectorAll(
    `[data-semester="${otherSemesterNumber}"]:not([data-optional-id],[data-is-training])`,
  );
  const yearHeaderElement = document.querySelector(
    `[data-header-year="${yearNumber}"]`,
  );

  if (actions.status === "on") {
    if (selectedCurrentSemester.length === allCurrentSemester.length)
      currentSubjectSemesterHeader.classList.add(
        modesMap[mode].subheader_selected_class,
      );

    if (
      selectedCurrentSemester.length + selectedOtherSemester.length ===
      allCurrentSemester.length + allOtherSemester.length - 1
    )
    ...
    if (selectedCurrentSemester.length === allCurrentSemester.length - 1)
      currentSubjectSemesterHeader.classList.remove(
        modesMap[mode].subheader_selected_class,
      );

    if (
      selectedCurrentSemester.length + selectedOtherSemester.length ===
      allCurrentSemester.length + allOtherSemester.length
    )
...
const modesMap = {
  ...
  second: {
    ...
    subheader_to_be_selected_class:
...
function toggleGPACell(actions) {
  ...
  const {
    dataset: { subjectId, creditHours },
    classList,
  } = actions.element;

  if (status === "on") {
    gpaDetails[modesMap[mode].gpaDetailsReserve][subjectId] =
      Number(creditHours);
...
function togglecolumnGPACell(actions) {
  ...
  const otherMode =
    mode === "first" ? "second" : mode === "second" ? "first" : "";
  const areOtherModesApplied = classList.contains(
    modesMap[otherMode][`${selectType}_selected_class`],
  );

  if (!otherMode || areOtherModesApplied) return;

  if (status === "on")
    classList.add(modesMap[mode][`${selectType}_selected_class`]);
  else classList.remove(modesMap[mode][`${selectType}_selected_class`]);
...
function toggleAllRelatedCells(actions) {
  ...
  const selectedClass =
    actions.status === "off" ? `.${modesMap[mode].cell_selected_class}` : "";
  const secondSemesterSelector = `, ${selectedClass}[data-semester="${secSem}"]:not([data-optional-id],[data-is-training])`;
  const semesterSubjects = document.querySelectorAll(
    `${selectedClass}[data-semester="${actions.selectType === "header" ? firstSem : headerSemester}"]:not([data-optional-id],[data-is-training])${actions.selectType === "header" ? secondSemesterSelector : ""}`,
  );
  const semesterSubHeaders = document.querySelectorAll(
    `[data-header-semester="${firstSem}"], [data-header-semester="${secSem}"]`,
  );

  semesterSubjects.forEach((subject) => {
    const subjectActions = { ...actions, selectType: "cell", element: subject };
    if (
      !subject.dataset.gpaSelectMode ||
      subject.dataset.gpaSelectMode === mode
    )
...
function toggleYearBasedOnSemesters(actions) {
  const { mode } = actions;
  const {
    dataset: { headerSemester },
    classList,
  } = actions.element;
  const year = Math.trunc((Number(headerSemester) + 1) / 2);
  const yearHeader = document.querySelector(`[data-header-year="${year}"]`);
  const otherSemester =
    Number(headerSemester) === year * 2 - 1 ? year * 2 : year * 2 - 1;
  const otherSemesterHeader = document.querySelector(
    `[data-header-semester="${otherSemester}"]`,
  );
  const isSemesterSelected = classList.contains(
    modesMap[mode].subheader_selected_class,
  );
  const isOtherSemesterSelected = otherSemesterHeader.classList.contains(
    modesMap[mode].subheader_selected_class,
  );
...
function isElementToggled(action) {
  ...
  const styleClasses = Object.keys(modesMap).map(
    (mapMode) => modesMap[mapMode][`${selectType}_selected_class`],
  );

  return !styleClasses.reduce(
    (res, styleClass) => (res &= !classList.contains(styleClass)),
    true,
  );
}
...
function handleGPASelect(event) {
  ...
  if (gpaSelectMode && actions.mode !== gpaSelectMode) return;
  ...
  if (actions.mode === "first") addSubjectsToCalculator();
  else if (actions.mode === "second") updatePreviousCompletedHours();
}

function updatePreviousCompletedHours() {
  const credits_input = document.getElementsByName("cgpa-credits")[0];

  credits_input.value = Object.keys(gpaDetails.oldSubjects).reduce(
    (sum, subject) => sum + gpaDetails.oldSubjects[subject],
    0,
  );
}
...
function addSubjectsToCalculator() {
  ...
  Object.keys(gpaDetails.currentSubjects).forEach((subject) => {
    ...
    } = MirectedGraph.search(graph, subject);
    ...
    const optionsText =
      '<option disabled>Choose</option> <option value="4.0">A+</option> <option value="4.0" selected>A</option> <option value="3.7">A-</option> <option value="3.3">B+</option> <option value="3.0">B</option> <option value="2.7">B-</option> <option value="2.3">C+</option> <option value="2.0">C</option> <option value="1.7">C-</option> <option value="1.3">D+</option> <option value="1.0">D</option> <option value="0.0">F</option>';
...
function resetCalculator(rows) {
  const headingText =
    '<div id="gpa-subject-headings"><div class="gpa-subject-heading"><b>Subject</b></div><div class="gpa-subject-heading"><b>Code</b></div><div class="gpa-subject-heading"><b>Credit hours</b></div><div class="gpa-subject-heading"><b>Grade</b></div></div>';
...
function toggleTotalGPAClick() {
  ...
  if (!CGPA_VIEW) {
    ...
    credits_input.style.borderBottom = undefined;
...
function toggleConfusedCGPAView() {
  ...
  if (!CGPA_VIEW) toggleTotalGPAClick();
...
function updateNGPAView(result) {
  ...
  const score = document.getElementById("ngpa-score");
  const cgpaCredits = Number(
    document.getElementsByName("cgpa-credits")[0].value,
  );
  const old_gpa = document.getElementsByName("cgpa")[0];
  ...
  const finalValue =
    (value * totalCredits + oldGpa * cgpaCredits) /
...
function calculateNGPA() {
  ...
  const gpa_subjects = Array.from(
    document.getElementsByClassName("gpa-select"),
  );
