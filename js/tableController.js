...
function initTable(subjects) {
  ...
  let prevCellPtr = currentCellPtr;
  let currentRowPtr = 1;
  const semestersCreditHours = [];
  ...
  subjects.forEach((semester) => {
    ...
    semester.forEach((subject) => {
      ...
      if (subject.type === TRAINING) isTrainingSem = true;
...
function getMaxTableRowLength(subjects) {
  ...
  subjects.forEach((semester) => {
    semester.forEach((subject) => {
      ...
      if (subject.optionalSet) counter += subject.optionalSet.length;
      else counter++;
    });
